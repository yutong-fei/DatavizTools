...
df_count = df.groupby(['the first column', 'the second column']).size().reset_index(name='count')
df_pivot = df_count.pivot(index='the first column', columns='the second column', values='count').fillna(0)
...
ax.set_title("Distribution of the second column across Identified Categories of the first column", fontsize=16)
ax.set_xlabel("Categories of the first column", fontsize=14)
